...
DSL 規格會被 UI Builder 讀取產生介面，因此以下內容必須在 DSL 中明確定義：

### Query 功能的搜尋規範

**當 Feature 名稱包含「查詢」時，必須定義搜尋行為：**
...
**每個刪除 Feature 必須明確定義刪除模式：**

```gherkin
# 在 Feature 頭部標註刪除模式
# @delete-mode: single | batch | both
...
Rule: 支援批次刪除多筆資料
  ...
  Example: 批次刪除時部分失敗
    Given 系統中存在球隊 "藍鷹隊"，建立者為 "coach1"
    And 系統中存在球隊 "紅龍隊"，建立者為 "coach2"
    And 使用者為「教練」角色，帳號為 "coach1"
    When 使用者 批次刪除球隊 ["藍鷹隊", "紅龍隊"]
    Then 應刪除 1 筆球隊（藍鷹隊）
    And 應回傳錯誤 "1 筆刪除失敗：無權限操作紅龍隊"
```
...
Rule: 支援分頁查詢，預設每頁 20 筆
  ...
  Example: 自訂每頁筆數
    When 使用者 查詢球員列表，每頁 50 筆
    Then 每頁筆數為 50

Rule: 每頁筆數選項為 10, 20, 50
```
...
Rule: 列表預設依建立時間倒序排列
  Example: 預設排序
    Given 球員 "王小明" 建立於 2026-01-01
    And 球員 "李大華" 建立於 2026-01-02
    When 使用者 查詢球員列表
    Then 第一筆為 "李大華"
    And 第二筆為 "王小明"

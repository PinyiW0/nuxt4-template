...
| 檔案 | 用途 | 產出時機 |
|------|------|----------|
| `glossary.json` | 跨 Epic 詞彙一致性 | Stage 1 |
| `boundary-decisions.json` | 避免重複決策 | Phase 1/2 確認後 |
| `*.feature` | **主要產出** | Stage 6 |

### 不產出檔案（保留在對話記憶）

| 階段 | 說明 |
|------|------|
| Stage 0 PRD 結構 | 一次性分析，不需持久化 |
| Stage 2 Feature 依賴 | 用 @requires/@publishes 標籤取代 |
| Stage 3 Events | 中間產物，直接用於 Stage 6 |
| Stage 4 Commands | 中間產物，直接用於 Stage 6 |
| Stage 5 Policies | 中間產物，直接用於 Stage 6 |

### Stage 7 視覺化（可選）

如需視覺化，Stage 7 會：
1. 從對話記憶或 .feature 檔案萃取資訊
2. 必要時補產 `events/*.json` 和 `commands/*.json`
3. 產出 PlantUML 圖表和詞彙對應表

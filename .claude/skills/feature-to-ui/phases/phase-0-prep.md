...
| Feature 類型 | 路由推導 | 說明 |
|-------------|---------|------|
| （無 feature 對應） | `/` → `index.vue` | **必建**：根路由，`navigateTo` 到第一個主要頁面 |
...
> ⚠️ **根路由必建**：即使沒有 feature 對應 `/`，route-map.yaml 也必須包含 `/` 路由，用 `navigateTo` 導向第一個主要頁面（如 `/trainings`）。登入成功後通常 `router.push('/')`，若 `/` 不存在會 404。

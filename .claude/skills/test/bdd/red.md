...
生成 Step Definition 後，**必須執行 lint 修復並確認零錯誤**：
...
常見需手動修的問題：
- `unused-imports/no-unused-vars`：未使用參數加 `_` 前綴（如 `world` → `_world`）
- `prefer-const`：`let` 改為 `const`
- `regexp/no-super-linear-backtracking`：改用更精確的 regex pattern

> **重要**：子代理（subagent）生成的程式碼也必須通過 lint。不通過 lint 的程式碼會導致 pre-commit hook 失敗，無法 commit。

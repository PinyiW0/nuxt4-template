...
const navigation = [
  { label: '訓練管理', icon: 'i-heroicons-clipboard-document-list', to: '/trainings' },
  { label: '歷史訓練', icon: 'i-heroicons-clock', to: '/history' },
  { label: '球隊管理', icon: 'i-heroicons-user-group', to: '/teams' },
  { label: '球員管理', icon: 'i-heroicons-users', to: '/players' },
  { label: '數據分析', icon: 'i-heroicons-chart-bar', to: '/analytics' },
]
  ...
  <div class="flex h-screen overflow-hidden bg-neutral-100 dark:bg-neutral-950">
    ...
    >
      ...
      <nav class="flex-1 space-y-1 overflow-y-auto p-2">
        <UTooltip v-for="item in navigation" :key="item.to" :text="item.label" :disabled="!isCollapsed" side="right">
          <NuxtLink
            :to="item.to"
      ...
      <div class="shrink-0 border-t border-neutral-200 p-2 dark:border-neutral-800">
        <!-- 深淺模式切換 -->
        <UTooltip :text="colorMode.value === 'dark' ? '淺色模式' : '深色模式'" :disabled="!isCollapsed" side="right">
          <button
        ...
        <!-- 會員名稱 + 登出（展開時） -->
        <div v-if="!isCollapsed" data-testid="user-menu" class="flex items-center gap-3 rounded-lg px-3 py-2">
          <UIcon name="i-heroicons-user-circle" class="size-5 shrink-0 text-primary-600 dark:text-primary-400" />
          <span data-testid="welcome-message" class="flex-1 truncate text-sm text-neutral-700 dark:text-neutral-300">
        ...
        <!-- 收合時：垂直排列 + Tooltip 提示 -->
        <div v-else data-testid="user-menu" class="flex flex-col items-center gap-1">
          <UTooltip :text="authStore.user?.account ?? '未登入'" side="right">
            <UIcon name="i-heroicons-user-circle" class="size-5 text-primary-600 dark:text-primary-400" />
          </UTooltip>
          <UTooltip text="登出" side="right">
            <UButton
              data-testid="logout-button"
              icon="i-heroicons-arrow-right-on-rectangle"
              color="neutral"
      ...
      <template #content>
        <div class="flex h-full flex-col bg-white dark:bg-neutral-900">
          ...
          <div class="shrink-0 border-t border-neutral-200 p-4 dark:border-neutral-800">
            ...
            <div data-testid="user-menu" class="flex items-center gap-3 rounded-lg px-3 py-2">
              <UIcon name="i-heroicons-user-circle" class="size-5 text-primary-600 dark:text-primary-400" />
              <span data-testid="welcome-message" class="flex-1 truncate text-sm text-neutral-700 dark:text-neutral-300">
                {{ authStore.user?.account ?? '未登入' }}
              </span>
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      <!-- Mobile Top Bar（in-flow，不使用 fixed/absolute） -->
